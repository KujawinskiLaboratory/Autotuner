...
test_that(desc = "Auotuner getters check",
          code = {
              ...
              expect_equal(class(peak_difference) == "data.frame", TRUE)
              expect_equal(nrow(metadata), 3)
              expect_equal(class(metadata) == "data.frame", TRUE)
              expect_equal(length(file_paths), 3)
              expect_equal(class(file_paths) == "character", TRUE)
              expect_equal(length(file_col), 1)
              expect_equal(class(file_col) == "character", TRUE)
              expect_equal(length(factorCol), 1)
              expect_equal(class(factorCol) == "character", TRUE)
          })
